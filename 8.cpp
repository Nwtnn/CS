...
std::tuple<int, int, int> extended_euclidean_gcd(int a, int b) {
    ...
    std::tuple<int, int, int> result = extended_euclidean_gcd(b, a % b);
    
    int gcd = std::get<0>(result);
    int x1 = std::get<1>(result);
    int y1 = std::get<2>(result);
...
int main() {
    ...
    if (a == 0 || b == 0) {
        std::cout << "GCD cannot be calculated for zero values. Please enter non-zero values." << std::endl;
        return 1;
    }
    ...
    std::tuple<int, int, int> extended_result = extended_euclidean_gcd(a, b);
    int extGcd = std::get<0>(extended_result);
    int x = std::get<1>(extended_result);
    int y = std::get<2>(extended_result);
